...
- `systolic-lob-feature-engine` (Systolic LOB Feature Engine: DeepLOB anchored public-safe matrix/vector feature projection with cycle estimate and limitations; not a production trading system)
...
- `session-high-low-breakout-validator` (Session High/Low Breakout Validator: public-safe Premarket, regular session, and close-range breakout validation with close-location value, VWAP distance, and volume pressure evidence; not financial advice and not a production trading system)
- `risk-adjusted-trade-sizing-engine` (Risk-Adjusted Trade Sizing Engine: public-safe Kelly-capped, drawdown-capped, volatility-scaled sizing report with risk budget, position debit, and dollars at risk evidence)
...
- `udp-control-plane-safety-monitor` (UDP Control Plane Safety Monitor: Precision Time Protocol references anchored public-safe Packet loss, sequence gap, stale command, and operator override behavior with rejected command and operator override evidence; not financial advice and not a production trading system)
- `sensor-fusion-market-risk-bridge` (Sensor Fusion Market Risk Bridge: Kalman filtering and state estimation notes anchored public-safe Noisy sensor analogy connected to noisy market state estimation with Kalman gain, posterior variance, low-trust sensor, and residual risk evidence; not financial advice and not a production trading system)
- `timing-attack-incident-runbook` (Timing Attack Incident Runbook: Precision Time Protocol references anchored public-safe Attack, detection, severity, mitigation, and postmortem template with escalation, operator communication bridge, and trusted-source recovery evidence; not financial advice and not a production trading system)
- `evidence-ledger-dashboard` (Evidence Ledger Dashboard: Model-risk and evidence-gating notes anchored public-safe Capability-to-proof dashboard with missing-evidence warnings, owner coverage, rejected proof, and blocked verdict evidence; not financial advice and not a production trading system)
- `recruiter-review-path-generator` (Recruiter Review Path Generator: Internal role packet docs anchored public-safe Role-specific artifact path generated from project metadata with selected repositories, review steps, missing link warnings, and coverage score evidence; not financial advice and not a production trading system)
