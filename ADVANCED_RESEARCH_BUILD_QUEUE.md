...
| Market simulation | Exchange Microburst Simulator (`exchange-microburst-simulator`) | Market infrastructure | Market Data Infrastructure Whitepaper | public-safe exchange microburst simulator for queue pressure, tail latency, packet drops, capacity planning, market infrastructure review, and operator actions; not financial advice and not a production trading system |
| Order entry | OUCH Order Entry Gateway Simulator (`ouch-order-entry-gateway-simulator`) | Market infrastructure / low-latency gateway systems | Market Data Infrastructure Whitepaper | public-safe OUCH-style order entry simulator for session sequencing, risk rejection, throttle pressure, ack latency, order gateway review, market infrastructure, and operator actions; not financial advice, not a production trading system, and not a real OUCH implementation |
| Post-trade reconciliation | Drop-Copy Reconciliation Engine (`drop-copy-reconciliation-engine`) | Market infrastructure / operations engineering | Market Data Infrastructure Whitepaper | public-safe drop-copy reconciliation engine for gateway decisions, execution reports, quantity mismatch detection, sequence gaps, duplicate execution report detection, operator escalation, and market infrastructure break reports; not financial advice, not a production trading system, and not connected to a broker or exchange |
| Session liveness | Session Heartbeat Failover Monitor (`session-heartbeat-failover-monitor`) | Market infrastructure / reliability engineering | Market Data Infrastructure Whitepaper | public-safe session heartbeat monitor for session liveness, heartbeat gaps, primary backup failover, split-brain warnings, failover SLA review, operator escalation, and market infrastructure session health; not financial advice, not a production trading system, and not connected to a broker or exchange |
| Integrated market infrastructure | Low-Latency Trading System Capstone (`low-latency-trading-system-capstone`) | Hardware / FPGA engineering, low-latency software, market infrastructure | Market Data Infrastructure Whitepaper / FPGA Trading Architecture Whitepaper | public-safe integrated market infrastructure capstone tying market data feed handler, sequence-gap detection, limit order book reconstruction, pre-trade risk gate, order gateway, drop-copy reconciliation, session heartbeat failover, deterministic replay, and latency budget evidence together; not financial advice, not a production trading system, and not connected to a broker or exchange |
| LOB intelligence | LOB Alpha Decay Research Lab (`lob-alpha-decay-research-lab`) | Quant developer / microstructure research | Strategy Robustness Whitepaper | public-safe limit order book research lab for signal half-life, horizon decay, cost-adjusted edge, information coefficient, and turnover cost drag; not financial advice and not a production trading system |
